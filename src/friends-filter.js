...
const getFriends = () => {
  return new Promise((resolve, reject) => {
    VK.api('friends.get', { v: '5.8', name_case: 'nom', fields: 'first_name, last_name, photo_50' }, response => {
      if (response.error) {
        reject(new Error(response.error.error_msg));
      } else {
        let {
...
const renderFiltered = (arr, input, container) => {
  const filteredFriends = arr.filter(friend => isMatching(`${friend.first_name} ${friend.last_name}`, input.value));

  renderFriends(filteredFriends, container);
};
const findBoundInput = zone => {
  switch (zone.dataset.bind) {
    case allFriendsInput.dataset.bind:
      return allFriendsInput;
    case filteredFriendsInput.dataset.bind:
      return filteredFriendsInput;
  }
};
